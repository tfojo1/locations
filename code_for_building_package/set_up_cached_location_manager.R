...
if (!INCLUDE.ZIP.CODES)
{
    print("NB: We are NOT including zip codes in the location manager")
    register.types(c(county.type,            cbsa.type,            state.type), #Typename
                   c(county.prefix,          cbsa.prefix,          state.prefix), #Prefix
                   c(county.prefix.longform, cbsa.prefix.longform, state.prefix.longform)) #Longform Name
}
DATA.DIR = 'data-raw'
